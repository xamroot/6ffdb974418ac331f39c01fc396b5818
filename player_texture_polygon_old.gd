...
func _draw():
	var pts = points
	poly.color = Color(0, 0, 1, 0.7)  # semi-transparent fill
	poly.polygon = pts
	draw_colored_polygon(pts, Color(0, 0.6, 1, 0.6))  # fill

func _process(_dt):
	queue_redraw()  # if you’re changing pts at runtime
...
func _physics_process(delta: float) -> void:
	...
	if growth_counter > 0.0:
		while processed < to_process and not frontier_data.is_empty():
			...
			# Claim this pixel if not yet owned (queued ensures we only see it once)
			var _new_points := PackedVector2Array()
			var _speed_break_threshold = 0.1
			for pt in points:
				var _dir = -pt.direction_to(center).normalized()
				var speed = 10.0
				while speed > _speed_break_threshold:
					
					var _new_point = pt + _dir*speed*delta
					if img.get_pixelv(Vector2i(_new_point))[0] > 0.419:
						_new_points.append(_new_point)
						break
					speed /= 2
				if speed < _speed_break_threshold:
					_new_points.append(pt)
				print(pt)
			points = _new_points
			break
